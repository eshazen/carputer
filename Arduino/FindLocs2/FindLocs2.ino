//
// Second generation display code
//
// read GPS lat/long
// display GMT time and nearest places
//
// use files:
//   config.csv    - configuration file, specifies names of place groups
//   grid.csv      - one line of grid spacing/range info
// for each place group
//   <name>.csv    - places annotated with grid coordinates
//   <name>.idx    - 256x256 array of 32-bit binary integers
...
#define MAX_GROUPS 5
// length of an 8 char filename with null terminator
#define MAX_FILE 9
// maximum length of group name
#define G_MAX_NAME 10
typedef struct {
  byte type;
  char file[MAX_FILE];
  char name[G_MAX_NAME];
} a_group;
...
void setup() {
  ...
  while( fc.readBytesUntil( 0xa, buff, sizeof(buff)) && ngroups < MAX_GROUPS) {
    ...
    groups[ngroups].type = atoi( tokn[0]);
    strncpy( groups[ngroups].file, tokn[1], sizeof(groups[0].file));
  ...
  ng = parse_csv_line(buff, tokn, MAXT);
  if( ng != 8) {
#ifdef USE_OLED
    fill_buffer( oledBuf, 0);		// clear screen
